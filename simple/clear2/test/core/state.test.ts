...
describe("core/state", () => {
  ...
  describe("StateStore", () => {
    ...
    it("setState applies updater immutably", () => {
      ...
      store.setState((s) => ({
        ...s,
        settings: { ...s.settings, model: "wjs" },
      }));

      const after = store.getState();

      expect(before.settings.model).toBe("claude-sonnet-4-20250514");
      expect(after.settings.model).toBe("wjs");
    });
    ...
    it("subscribe notifies on state change", () => {
      ...
      store.setState((s) => ({
        ...s,
        settings: { ...s.settings, model: "opus" },
      }));
  ...
  describe("selectors", () => {
    ...
    it("isStreaming checks stream mode", () => {
      ...
      const streaming = {
        ...state,
        ui: { ...state.ui, streamMode: "streaming" as const },
      };
